...
@ef = [318258242717201709453901384328569236653, 75380722035796344355219475510170298006, 129416082603460579272847694630998099237, 143864072772599444046778416709082679388]

@pp = 2 ** 128 - 173
@rr = 115792089237316195429342203801033554170931615651881657307308068079702089951781
@ef[4] = [318258242717201709453901384328569236653, 75380722035796344355219475510170298006, 129416082603460579272847694630998099237, 143864072772599444046778416709082679388]
...
def g2add(u11, u10, u21, u20, v11, v10, v21, v20, f3, f2, f1, f0, p)
...
def g2dbl(u1, u0, v1, v0, f3, f2, f1, f0, p)
  ...
  l3 = (iv * (m4 * s1 - m1 * s2)) % p; l2 = (iv * (m0 * s2 - m3 * s1)) % p
  l1 = (v1 + u1 * l2 - (uu1 - u0) * l3) % p; l0 = (v0 + u0 * l2 - (uu0) * l3) % p
  ue1 = (2 * l3 * l2 - 2 * u1 - 1) % p
  ue0 = (2 * l3 * l1 + l2 * l2 - 2 * u0 - uu1 - 2 * ue1 * u1) % p
...
def mktable(u, v)
  ...
  for i in 1..N - 1 #begin Pub_key at plain
    #g2dbl(@uu[0], @uu[1], @vv[0], @vv[1] ,@ta[0], @ta[1], @ta[2], @ta[3], @p2)
    j2dbl(@uu[0], @uu[1], @vv[0], @vv[1],0,0,0,0, @ta[0], @ta[1], @ta[2], @ta[3], @test_p)
    @le_u[i] = @uu
    @le_v[i] = @vv
    print i, "=", @le_v[i], " , ", @le_u[i], "\n"
...
def jac(kk, p)
  ...
  for i in 1..j + 1
    ...
    #while(1)
    if (@U != @le_u[ki[i]])
      j2add(@U[0], @U[1], @le_u[ki[i]][0], @le_u[ki[i]][1], @V[0], @V[1], @le_v[ki[i]][0], @le_v[ki[i]][1],0,0,0,0, @FF[1], @FF[2], @FF[3], @FF[4], @P)
      #g2add(@U[1], @U[0], @le_u[ki[i]][1], @le_u[ki[i]][0], @V[1], @V[0], @le_v[ki[i]][1], @le_v[ki[i]][0], @ef[0], @ef[1], @ef[2], @ef[3], @p2)
      #print "ii=", i, " ", @le_u[ki[i]], " ,", @U, "\n"
    end
...
ua = [4479815356, 4309010080141014784]
va = [6183226511, 87584922866592430046826643578455405177]
=begin
j2add(@tu1[0],@tu1[1],
@tu2[0],@tu2[1],
@tv1[0],@tv1[1],
...
@ta[3],11)
=end
#j2add(7,10 ,0,10,1,9,7,9,0,0,0,0, 3,7,1,2,11)
#g2add(7,10 ,0,10,1,9,7,9, 3,7,1,2,11)
#mktable(@tu , @tv)
#jac(@Jga, @P)
