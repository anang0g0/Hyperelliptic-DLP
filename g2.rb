...
def a2add(u11, u10, v11, v10, u21, u20, v21, v20, f4, f3, f2, f1, f0, p)
  ...
  l1 = (u11 * l2 + v11 + (b0 - u10) * l3)%p;
  u31 = (2 * l2 * l3 + 1 - ss3)%p;
  u30 = (2 * l1 * l3 + l2 * l2 - f4 - u31 * ss3 - ss2)%p;
  ul0 = (u30 * l3)%p; ul = (-u31 * l2 + l1)%p;
  v31 = -(u31 * u31*l3 - ul0 + ul)%p; v30 = -(u31 * ul0 + ul)%p;
...
def mktable(u, v)
  ...
  for i in 1..N - 1 #begin Pub_key at plain
    g2dbl(@uu[0], @uu[1], @vv[0], @vv[1] ,@FF[0], @FF[1], @FF[2], @FF[3], @P)
    @le_u[i] = @uu
    @le_v[i] = @vv
...
def jac(kk, p)
  ...
  #while(1)
    if (@U != @le_u[ki[i]])
      g2add(@U[1], @U[0], @le_u[ki[i]][1], @le_u[ki[i]][0], @V[1], @V[0], @le_v[ki[i]][1], @le_v[ki[i]][0], @FF[0], @FF[0], @FF[2], @FF[3], @P)
