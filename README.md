...
コンパイル方法：本体は、cpp/ZZ/jacobian.cppです。
jacobian.cppに対して、

make

してあります。
デフォルトはclang++ですが、これはg++だとコンパイルに時間がかかるからです。
...
ここまで短期間で実装できたのは、私がMcEliece暗号のために2年間費やして作った多項式ライブラリのおかげで、この多項式ライブラリが応用に使えるという証拠にもなった。

ちなみにここで実装しているのはカントールアルゴリズムです。

巨大整数を使うために、NTL6.2を使っています。
